...
class Settings:
    ...
    def load(cls, path: Path = CONFIG_PATH) -> "Settings":
        ...
        s.brain_db_path   = raw.get("brain", {}).get("db_path", s.brain_db_path)
        s.memory_dir      = raw.get("memory", {}).get("dir", s.memory_dir)
        s.watch_folders   = raw.get("brain", {}).get("watch_folders", [])
        s.pc_bridge_port  = raw.get("pc_bridge", {}).get("port", 8765)
        s.claws_dir       = raw.get("claws_dir", s.claws_dir)
        s.tools_experimental_dir = raw.get("tools_experimental_dir", s.tools_experimental_dir)
        s.tools_active_dir       = raw.get("tools_active_dir", s.tools_active_dir)
        s.exports_dir     = raw.get("exports_dir", s.exports_dir)
        s.logs_dir        = raw.get("logs_dir", s.logs_dir)
