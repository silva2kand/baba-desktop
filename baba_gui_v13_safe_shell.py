...
class BabaGuiV13SafeShell(tk.Tk):
    def __init__(self) -> None:
        ...
        self.claws = (
            ClawInstaller(str(APP_DIR / "src" / "claws" / "installed"))
            if ClawInstaller
            else None
        )
    ...
    def _build_sidebar(self) -> None:
        ...
        items = [
            ("💬", "Chat Ready", lambda: self._emit_system("Chat is active.")),
            (
                "🔗",
                "Connections",
                lambda: self._emit_system(
                    "Use Connection Hub + Approvals to connect providers/apps."
                ),
            ),
            ("🩺", "Self Check", self._run_startup_self_check),
            (
                "🔄",
                "Refresh Detect",
                lambda: self._scan_connections_now(async_mode=True),
            ),
        ]
    ...
    def _build_main_chat(self) -> None:
        ...
        tk.Button(
            ...
            bd=0,
            font=("Segoe UI", 16),
            command=lambda: self._emit_system(
                "Attachments are disabled in this safe-shell build."
            ),
        ...
        def worker() -> None:
            try:
                if action == "connect_resource":
                    ...
                    skill_name = payload.get("skill_name", "custom_skill")
                    description = payload.get("description", "Custom skill")
                    auto_promote = bool(payload.get("auto_promote", True))
                    builder = self._ensure_tool_builder()
                    if not builder:
                        self.after(0, lambda: self._emit_system("Tool builder is unavailable in this run."))
                        return
                    draft = asyncio.run(builder.build_skill_from_description(description, name=skill_name))
                    if not draft.get("ok"):
                        self.after(0, lambda d=draft: self._append_chat("Builder", f"Skill build failed: {d.get('error', 'unknown error')}"))
                        return
                    summary = f"Built draft skill: {skill_name}"
                    if auto_promote:
                        promoted = builder.promote_skill(skill_name, approved=True)
                        summary += f". {promoted.get('message', '')}"
                    self.after(0, lambda s=summary: self._append_chat("Builder", s))
                else:
    ...
    def _approve_all_apps(self) -> None:
        approved = []
        for name, meta in self.connection_live.get("apps", {}).items():
            if meta.get("available"):
                self._approve_connection("apps", name)
                approved.append(name)
        if approved:
            self._emit_system(
                f"Approved detected apps: {', '.join(sorted(approved))}."
            )
        else:
            self._emit_system("No detected apps available to approve.")

    def _require_approval_or_prompt(self, kind: str, name: str) -> bool:
        meta = self.connection_live.get(kind, {}).get(name, {})
        if not meta.get("available"):
            self._emit_system(f"{name} is not detected/available on this machine.")
            return False
    ...
    def _open_target(self, target: str) -> None:
        ...
        try:
            ...
            if t in appbridge_required and not self.apps:
                result = {
                    "ok": False,
                    "error": "AppBridge backend is not available in this build/run.",
                }
            elif t in {"whatsapp", "gmail", "outlook", "facebook", "telegram", "tiktok", "instagram", "x", "exo"} and self.apps:
    ...
    def _parse_local_command(self, text: str) -> Tuple[bool, str]:
        ...
        if low in {"help", "capabilities", "what can you do", "what it can do"}:
            return True, (
                ...
                "- Build reusable skills with your approval: build skill <name>\n"
        ...
        if low in {"self check", "health check", "startup check", "status features"}:
            return True, self._capability_summary_text()
        ...
        if "install claw" in low or "enable claw" in low:
            if not self.claws:
                return True, "Claw installer backend is missing in this run."
            claw_id = ""
        ...
        if low.startswith("build tool ") or "build a tool" in low or "create tool" in low:
            if ToolBuilder is None:
                return True, "Tool builder is not installed/available in this run."
        ...
        if low.startswith("build skill ") or "build a skill" in low or "create skill" in low:
            if ToolBuilder is None:
                return True, "Skill builder is not installed/available in this run."
            raw_name = (
                low.replace("build skill", "").replace("create skill", "").strip()
            )
            skill_name = "".join(ch if ch.isalnum() or ch == "_" else "_" for ch in raw_name).strip("_")[:40] or f"skill_{int(time.time())}"
            description = text.strip()
            self._enqueue_approval(
                title=f"Build skill: {skill_name}",
                action="build_skill",
                payload={"skill_name": skill_name, "description": description, "auto_promote": True},
                detail="Will generate and promote a reusable skill definition.",
            )
            return True, f"Approval requested to build skill '{skill_name}'."
        ...
        if any(k in low for k in ("create video", "make video", "generate video")):
            ...
            if ToolBuilder is None:
                return (
                    True,
                    "Video creator tool missing, and tool builder is unavailable in this run.",
                )
            self._enqueue_approval(
    ...
    def _compute_capabilities(self) -> Dict[str, Any]:
        ai = {}
        for p in ("ollama", "jan", "lmstudio"):
            on, _ = _test_provider(p)
            ai[p] = bool(on)
        return {
            "ai_providers": ai,
            "app_bridge": bool(self.apps),
            "claw_installer": bool(self.claws),
            "tool_builder": bool(ToolBuilder is not None),
            "script_tools_path": bool((APP_DIR / "src" / "tools").exists()),
        }

    def _capability_summary_text(self) -> str:
        caps = self.capabilities or self._compute_capabilities()
        ai = caps.get("ai_providers", {})
        ai_line = ", ".join(
            f"{k}:{'ON' if v else 'OFF'}" for k, v in ai.items()
        )
        return (
            "Startup Capability Check\n"
            f"- Local AI: {ai_line}\n"
            f"- AppBridge: {'ON' if caps.get('app_bridge') else 'OFF'}\n"
            f"- Claw Installer: {'ON' if caps.get('claw_installer') else 'OFF'}\n"
            f"- Tool Builder: {'ON' if caps.get('tool_builder') else 'OFF'}\n"
            f"- Script Tools Path: {'ON' if caps.get('script_tools_path') else 'OFF'}\n"
        )

    def _run_startup_self_check(self) -> None:
        self.capabilities = self._compute_capabilities()
        self._emit_system(self._capability_summary_text())
