...
class BabaDesktop(tk.Tk):
        ...
        try:
            if self.pool and self.brain:
                self.tool_builder = ToolBuilder(
                    self.pool, self.brain, self.settings, self.memory
                )
    ...
    def _tool_builder_display(self, result):
        text = (
            json.dumps(result, indent=2)
            if isinstance(result, (dict, list))
            else str(result)
        )
        self._log(f"Tool Builder: {text[:200]}")
        if hasattr(self, "evolve_output") and self.evolve_output.winfo_exists():
            self._evolve_append(f"\n[Tool Builder]\n{text}\n")
    ...
    def _panel_evolve(self):
        ...
        af = tk.Frame(frame, bg=T["bg"])
        af.pack(fill="x", padx=20, pady=(0, 8))
        tk.Button(
            af,
            text="Propose Updates",
            font=FONT_SM,
            bg=T["panel2"],
            fg=T["accent"],
            bd=0,
            ...
            pady=4,
            cursor="hand2",
            command=self._evolve_propose_updates,
        ).pack(side="left", padx=4)
        tk.Button(
            af,
            text="Build Tool Draft",
            font=FONT_SM,
            bg=T["panel2"],
            fg=T["accent"],
            bd=0,
            ...
            pady=4,
            cursor="hand2",
            command=self._evolve_build_tool,
        ).pack(side="left", padx=4)
        tk.Button(
            af,
            text="Build Skill Draft",
            font=FONT_SM,
            bg=T["panel2"],
            fg=T["accent"],
            bd=0,
            ...
            pady=4,
            cursor="hand2",
            command=self._evolve_build_skill,
        ).pack(side="left", padx=4)
        tk.Button(
            af,
            text="Queue Knowledge",
            font=FONT_SM,
            bg=T["panel2"],
            fg=T["accent"],
            bd=0,
            ...
            pady=4,
            cursor="hand2",
            command=self._evolve_queue_knowledge,
        ).pack(side="left", padx=4)
        tk.Button(
            af,
            text="Review Pending",
            ...
            pady=4,
            cursor="hand2",
            command=self._evolve_review_updates,
        ).pack(side="left", padx=4)
        ...
        self.evolve_output.insert(
            "end",
            "- Records interactions and decisions\n"
            "- Builds tool/skill drafts\n"
            "- Queues every system update for your approval\n"
            "- Applies only approved updates\n\n",
        )
        if self.memory:
    ...
    def _evolve_append(self, text):
        if not hasattr(self, "evolve_output") or not self.evolve_output.winfo_exists():
            self._log(text[:120])
            return
        self.evolve_output.configure(state="normal")
        self.evolve_output.insert("end", text)
        self.evolve_output.see("end")
        self.evolve_output.configure(state="disabled")

    def _evolve_propose_updates(self):
        if not self.tool_builder:
            self._evolve_append("\nTool Builder not available.\n")
            return
        goal = simpledialog.askstring(
            "Self-Improve",
            "Optional goal/focus area:",
            initialvalue="Improve skills, memory and safe automation",
        )
        if goal is None:
            return
        ...
        def _do():
            ...
            try:
                items = asyncio.run(
                    self.tool_builder.propose_self_improvements(goal=goal, limit=8)
                )
                if not items:
                    out = "No proposals generated.\n"
                else:
                    lines = ["Proposed updates:"]
                    for i, item in enumerate(items, 1):
                        lines.append(
                            f"{i}. [{item.get('kind')}] {item.get('name')} - {item.get('summary')}"
                        )
                    out = "\n".join(lines) + "\n"
            except Exception as e:
                out = f"Proposal error: {e}\n"
            self.after(0, lambda: self._evolve_append(f"{out}\n"))

        threading.Thread(target=_do, daemon=True).start()

    def _evolve_build_tool(self):
        if not self.tool_builder:
            self._evolve_append("\nTool Builder not available.\n")
            return
        description = simpledialog.askstring(
            "Build Tool", "Describe the tool to build:"
        )
        if not description:
            return
        name = simpledialog.askstring(
            "Build Tool", "Tool name (optional snake_case):", initialvalue=""
        )
        self._evolve_append("\nBuilding tool draft...\n")
        ...
        def _do():
            ...
            try:
                draft = asyncio.run(
                    self.tool_builder.build_from_description(description, name=name or None)
                )
                if not draft.get("ok"):
                    out = f"Build failed: {draft.get('error', 'unknown error')}\n"
                else:
                    req = self.tool_builder.queue_update(
                        "promote_tool",
                    ...
                    out = (
                        f"Draft created: {draft['name']}\n"
                        f"Pending approval request: {req.get('id')}\n"
                    )
            except Exception as e:
                out = f"Build error: {e}\n"
            self.after(0, lambda: self._evolve_append(out))

        threading.Thread(target=_do, daemon=True).start()

    def _evolve_build_skill(self):
        if not self.tool_builder:
            self._evolve_append("\nTool Builder not available.\n")
            return
        description = simpledialog.askstring(
            "Build Skill", "Describe the skill behavior:"
        )
        if not description:
            return
        name = simpledialog.askstring(
            "Build Skill", "Skill name (optional snake_case):", initialvalue=""
        )
        ...
        def _do():
            ...
            try:
                draft = asyncio.run(
                    self.tool_builder.build_skill_from_description(
                        description, name=name or None
                    )
                )
                if not draft.get("ok"):
                    out = f"Skill build failed: {draft.get('error', 'unknown error')}\n"
                else:
                    req = self.tool_builder.queue_update(
                        "promote_skill",
                    ...
                    out = (
                        f"Skill draft created: {draft['name']}\n"
                        f"Pending approval request: {req.get('id')}\n"
                    )
            except Exception as e:
                out = f"Skill build error: {e}\n"
            self.after(0, lambda: self._evolve_append(out))

        threading.Thread(target=_do, daemon=True).start()

    def _evolve_queue_knowledge(self):
        if not self.tool_builder:
            self._evolve_append("\nTool Builder not available.\n")
            return
        topic = simpledialog.askstring("Queue Knowledge", "Topic/title:")
        if not topic:
            return
        content = simpledialog.askstring("Queue Knowledge", "Knowledge content:")
        if not content:
            return
        source = simpledialog.askstring(
            "Queue Knowledge", "Source:", initialvalue="manual"
        )
        self.tool_builder.draft_knowledge_note(topic, content, source=source or "manual")
        req = self.tool_builder.queue_update(
            "save_knowledge",
            f"Save knowledge: {topic}",
            {"topic": topic, "content": content, "source": source or "manual"},
            summary=content[:120],
        )
        self._evolve_append(
            f"\nKnowledge draft queued.\nPending approval request: {req.get('id')}\n"
        )

    def _evolve_review_updates(self):
        if not self.tool_builder:
            self._evolve_append("\nTool Builder not available.\n")
            return
        pending = self.tool_builder.list_pending_updates()
        if not pending:
            self._evolve_append("\nNo pending updates.\n")
            return
        lines = [f"\nReviewing {len(pending)} pending updates...\n"]
        for req in pending:
            detail = (
            ...
            approved = messagebox.askyesno("Self-Improve Approval", detail)
            result = self.tool_builder.decide_update(req.get("id", ""), approved)
            lines.append(
                f"- {req.get('id')}: {'APPROVED' if approved else 'DENIED'} -> {result.get('status', 'done')}"
            )
        self._evolve_append("\n".join(lines) + "\n")
